...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    data = struct('out4',0);
    initialState = setupFcn(fig,handles);
    disp(data.out4);
...
function initialState = setupFcn(fig,handles)
...
% set(fig,'WindowButtonMotionFcn',@(o,e,handles) dummy()); % Add dummy so that the CurrentPoint is constantly updated
set(fig,'WindowButtonMotionFcn',{@mouse_trace,handles.edit_tini});
...
function mouse_trace(~,~,edit_box)
    
    x = get(gca, 'CurrentPoint');
    set(edit_box,'String',sprintf('%4.2f',x(1)));
